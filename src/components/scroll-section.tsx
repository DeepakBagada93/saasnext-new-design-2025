...
export function ScrollSection() {
  ...
  // Calculate which card is active. 3 problems + 1 solution = 4 total states.
  const cardIndex = useTransform(scrollYProgress, [0, 0.25, 0.5, 0.75, 1], [0, 1, 2, 3, 3]);
      ...
      <div className="sticky top-0 h-screen flex items-center justify-center overflow-hidden">
        <div className="relative flex w-full max-w-3xl mx-auto px-4">
          ...
          {problems.map((problem, index) => {
            const isActive = cardIndex.get() === index;
             return (
                 <motion.div
                    key={index}
                    className={cn(
                    ...
                    style={{
                        opacity: useTransform(scrollYProgress, [index * 0.2, index * 0.2 + 0.1, (index + 1) * 0.2 + 0.1, (index + 1) * 0.2 + 0.2], [0, 1, 1, 0]),
                        scale: useTransform(scrollYProgress, [index * 0.2, index * 0.2 + 0.1], [0.9, 1]),
                        y: '-50%',
                        x: '-50%',
                        top: '50%',
                        left: '50%',
          ...
          <motion.div
            ...
            style={{
                ...
                scale: useTransform(scrollYProgress, [0.75, 0.85], [0.8, 1]),
                y: '-50%',
                x: '-50%',
                top: '50%',
                left: '50%',
