    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Overview of your active and past projects.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {clientProjects.map(project => (
                <TableRow key={project.id}>
                  ...
                  <TableCell>{new Date(project.timeline.end).toLocaleDateString()}</TableCell>
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" asChild>
                        <Link href={`/client/projects/${project.id}`}>View Details <ArrowRight className="ml-2 h-4 w-4"/></Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
